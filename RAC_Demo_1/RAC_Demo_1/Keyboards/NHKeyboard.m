...
- (void)loadRandomNumber {
    ...
    [subviews enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[UIButton class]]) {
            ...
            NSString *title ;
            if (__tag == 9) {
                title = decimal?[titles objectAtIndex:__tag]:@"return";
            }else if (__tag == 10) {
...
- (void)setupASCIICapableLayout:(BOOL)init {
    ...
    if (init) {
        ...
        btn.titleLabel.textColor = titleColor;
        [btn setTitleColor:titleColor forState:UIControlStateNormal];
        [btn setTitle:self.shiftEnable?NHARROWDOWN:NHARROWUP forState:UIControlStateNormal];
...
- (void)updateShiftBtnTitleState {
    ...
    if (self.showSymbol) {
        ...
        title = self.shiftEnable?NHARROWDOWN:NHARROWUP;
    }
